...
def main():
    ...
    manager = HarborManager(h, replication_rule_prefix=options.rule_prefix, trigger_execution=options.trigger)
    ...
    if options.mode == 'create':
        ...
        process_func = partial(manager.replicate_image,
                               registry=registry)
    elif options.mode == 'delete':
        process_func = partial(manager.remove_image_replication,
                               delete_projects=options.delete_projects)
    else:
...
def parse_cli():
    ...
    parser.add_argument('-v', '--verbose', dest='verbosity', action="count", help="Verbose output")
    ...
    behaviour.add_argument('--registry', help="Name of the registry to use when creating new policies")
    behaviour.add_argument('--delete-projects', dest="delete_projects", action="store_true", help="Delete projects (and not just rules) if deleting")
...
class ReplicationPolicy:
    ...
    def update_filters(self, harbor_client):
        ...
        new_filters = [oldfilter for oldfilter in self._raw['filters'] if oldfilter['type'] != 'tag']
...
class HarborManager:
    def __init__(self, harbor_client, *, replication_rule_prefix, trigger_execution=True):
...
class HarborClient:
    ...
    def remove_project_content(self, name):
        ...
        for repo in repos:
            ...
            address = self.base_url + f"api/v2.0/projects/{name}/repositories/{repo_name}"
            response = self._session.delete(address, auth=self._credentials)
            logger.debug(f"Response headers: {response.headers}")
            response.raise_for_status()
